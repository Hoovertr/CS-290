...
function bindButtons(){
    document.getElementById('citySubmit').addEventListener('click', function(event){
		...
		var payload = {searchKey:null};
		if(document.getElementById('city').value != ""){
        	payload.searchKey = document.getElementById('city').value;
		} else {
			payload.searchKey = document.getElementById('zipc').value;
		}
        	req.open('GET', 'http://api.openweathermap.org/data/2.5/weather?q='+payload.searchKey+'&appid=' + apiKey+'&units=imperial', true);
